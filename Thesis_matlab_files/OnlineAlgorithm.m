function MyHypotheses = OnlineAlgorithm(Hypothesis, NewPoint, time)

% init_cov = var(NewPoint) /10 ;
init_cov = 3 ;
% merge_thresgold = 1e-2 ;
pp_extinction = 1 ;    pp_thresh = 0.01 ;
max_Hypothesis_K_record = 30 ;
...
..... Again calculate indicator to update loglike, it really helps us in building a stronger case
                   ...
                   for ii = 1 : k 
                    ...
                    indicator = semi_indicator.* updated_pp ;
                    indicator(indicator < 1e-84) = 1e-84 ;
                   new_loglike = Hypothesis(j).loglike + ( log( sum(indicator) ) ) ;  % realmin + sum(indicator)
                   dl_after_update = - new_loglike + ( nparsover2 * sum(log(updated_pp)) ) + ...
...
%   Add hypothesis : (New point as a new component) + (updated Hypothesis(1))
    clear mn Sigma ;
         ...
         Hypothesis(j+1).loglike = Hypothesis(1).loglike ;
...
%          updated_pp = check_weights( updated_pp ) ;
%          Hypothesis(j+1).prior = updated_pp ;      
         k = length( updated_pp ) ;
         clear N_k ;
         N_k = Hypothesis(j+1).Nk ;
%          N_k = sum(N_k) * updated_pp ;
%          Hypothesis(j+1).Nk = N_k ;
         dl_new_H = - Hypothesis(j+1).loglike + (nparsover2 * sum(log(updated_pp))) + ...
                    (nparsover2 + 0.5) * k * log( sum(N_k) ) ;%%%+k*(nparsover2+0.5)*(1-log(12));
...
...........................................................................
        merge_threshold = 0.5 * (time/(time+600)) ; % CTD data [0 , 5]
...
%             updated_pp = check_weights( updated_pp ) ;
            ...
            Hypothesis(L+1).loglike = Hypothesis(1).loglike ;            
            clear N_k ;
            N_k = Hypothesis(L+1).Nk ;
            k = length( updated_pp ) ;
            dl_Comp = - Hypothesis(L+1).loglike + ( nparsover2 * sum(log( updated_pp ))) + ...
                            (nparsover2 + 0.5) * k * log(sum( N_k ));%%%+k*(nparsover2+0.5)*(1-log(12));
    ...
    ....................................
      ...
      Hypothesis = Hypothesis1(idx_dl(1 : min( 110 , length(idx_dl) ))) ;
      clear Hypothesis1 ;
      clear idx_dl ;
      ...
