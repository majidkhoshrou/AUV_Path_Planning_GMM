function [Merged_Cluster ,merge_flag ] = Post_Merge_Pre_Merge_5( Cluster, nparsover2, merge_thresgold, time )
    ...
    Pre_individual_Cluster(i) = struct( 'mu',estmu(:,i),'cov',estcov(:,:,i),'prior', prior(i),'Nk',Nk(i) ) ;
            ...
            B_ij = Inf( num_clusters - 1 , num_clusters );
             for g = 1 : num_clusters
              ...
              IND = find( B_ij < merge_thresgold ) ;   %  IND = find(B_ij == min(nonzeros(B_ij))); 
                if isempty( IND )
                    Merged_Cluster = Cluster ;
                    merge_flag = 0 ; 
                    return ;
